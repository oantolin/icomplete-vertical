...
to =icomplete-vertical-do=: it should contain at least one newline; it
can have text properties, such as faces, which control the display;
and if it has no faces it will have =icomplete-vertical-separator= face
applied to it. For example, the following specification makes a red
dotted line:
